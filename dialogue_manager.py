...
class dialogue_manager:
    ...
    def load_core_elements_from_pickle(self):
        ...
        print "self.current_asp_confidence: " + str(self.current_asp_confidence)
        print "self.request_type: " + str(self.request_type)
        print "self.role_requested: " + str(self.role_requested)
        print "self.last_apr: " + str(self.last_apr)
        print "self.last_role_requested " + str(self.last_role_requested)
        print "self.roles_relatively_confident_about: " + str(self.roles_relatively_confident_about)
        print "self.current_best_asp_understanding: " + str(self.current_best_asp_understanding)
        print "self.utterances_for_goal: " + str(self.utterances_for_goal)
        print "self.utterances_for_clarification: " + str(self.utterances_for_clarification)
        print "self.utterances_during_dialogue: " + str(self.utterances_during_dialogue)
        print "self.dialogue_accepted_parses: " + str(self.dialogue_accepted_parses)
        f.close()
...
#                                                                       self.all_unmapped_words[unmapped_w][w] = unmapped_words_from_utterances[i][unmapped_w][w]
    ...
    def process_user_initiative_response(self, user_utterances, user_utterance_class):
        ...
        for i in range(0,len(user_utterances)):
            ...
            for j in range(0,len(parses)):
                ...
                #print "asp_node: " + str(asp_node)
                #print "utterance_scores: " + str(utterance_scores)
                #print "utterance_scores["+str(i)+"]: " + str(utterance_scores[i])
                #print "parses["+j+"]: " + str(parses[j])
        ...
        for i in range(0,len(asp_nodes)):
            ...
            for role in self.asp_role_map:
                for candidate in apr_tuple[self.asp_role_map[role]]:
                    ...
                    print "candidate : " + str(candidate) + "confidence: " + str(confidence)
        ...
        for role in self.asp_role_map:
            for existing_candidates in self.current_asp_confidence[self.asp_role_map[role]]:
                if (existing_candidates not in role_candidates[role]):
                    ...
                    print "unmentioned candidate: " + str(existing_candidates)
            print "Confidences for  " + str(role) + ": " + str(self.current_asp_confidence[self.asp_role_map[role]])
    ...
    def articulate_next_state(self):
        print "Confidences: " + str(self.current_asp_confidence)
        ...
        for role in self.asp_role_map:
            #print "self.current_best_asp_understanding[self.asp_role_map[role]][1]" + str(self.current_best_asp_understanding[self.asp_role_map[role]][1])
            #print "self.current_best_asp_understanding[self.asp_role_map[role]][0]" + str(self.current_best_asp_understanding[self.asp_role_map[role]][0])
            #print "role: " + str(role)
            print str(self.current_best_asp_understanding[self.asp_role_map[role]])
    ...
    def articulate_next_state_entr(self):
        print "*self.current_asp_confidence: " + str(self.current_asp_confidence)
        #use confidence scores to determine what to ask about
        ...
        print "DEBUG: articulate_next_state relative confidence after constraints="+str(relatively_confident_about) #DEBUG
        if (None in relatively_confident_about):
            rec_val = np.array(self.current_best_asp_understanding[self.asp_role_map["recipient"]])
            rec_conf = False
            for candidate in self.current_asp_confidence[self.asp_role_map["recipient"]]:
                if (self.current_asp_confidence[self.asp_role_map["recipient"]][candidate]>self.min_confidence_to_accept):
                        rec_conf = True
            act_val =   np.array(self.current_best_asp_understanding[self.asp_role_map["action"]])
            act_conf = False
            for candidate in self.current_asp_confidence[self.asp_role_map["action"]]:
                if (self.current_asp_confidence[self.asp_role_map["action"]][candidate]>self.min_confidence_to_accept):
                    act_conf = True
            pat_val = np.array(self.current_best_asp_understanding[self.asp_role_map["patient"]])
            pat_conf = False
            for candidate in self.current_asp_confidence[self.asp_role_map["patient"]]:
                    if (self.current_asp_confidence[self.asp_role_map["patient"]][candidate]>self.min_confidence_to_accept):
                        pat_conf = True
            if np.any(rec_val==False) and (rec_conf==False):
                self.role_requested = "recipient"
            elif np.any(act_val==False) and (act_conf==False):
                self.role_requested = "action"
            elif np.any(pat_val==False) and (pat_conf==False):
                self.role_requested = "patient"
            elif (relatively_confident_about[self.asp_role_map["action"]] == "query"): #can't clarify an unknown query argument
                verbal_query = "Sorry I couldn't understand that. Could you reword your question?"
                self.request_type = "user_initiative"
                self.role_requested = None
                return verbal_query,relatively_confident_about
            ...
            #self.role_requested = [role for role in self.asp_role_map if self.current_best_asp_understanding[self.asp_role_map[role]][1] == min_conf][0]
                ...
                role_array=[]
                for role in self.asp_role_map:
                    if len(self.current_asp_confidence[self.asp_role_map[role]])>1:
                        probs = []
                        entrs_to_sum = []
                        i = 0
                        confident = False
                        for candidate in self.current_asp_confidence[self.asp_role_map[role]]:
                            if (self.current_asp_confidence[self.asp_role_map[role]][candidate]>self.min_confidence_to_accept):
                                confident = True
                        if (confident==False):

                            conf_sum = np.sum(self.current_asp_confidence[self.asp_role_map[role]][candidate] for candidate in self.current_asp_confidence[self.asp_role_map[role]])
                            print "sum: " + str(conf_sum)
                
                            for candidate in self.current_asp_confidence[self.asp_role_map[role]]:
                                probs.append(self.current_asp_confidence[self.asp_role_map[role]][candidate]/conf_sum)
                            for item in probs:
                                entrs_to_sum.append(- item*math.log(item, 2))
                            print "probs: " + str(probs)
                            print "entrs_to_sum: " + str(entrs_to_sum)
                            role_array.append(role)
                            entr_array.append(sum(entrs_to_sum))

                print "entropies: " + str(entr_array)
                print "roles: " +str(role_array)
                if len(entr_array)>0:
                    self.role_requested = role_array[entr_array.index(max(entr_array))]
                else:
                    min_conf = min([self.current_best_asp_understanding[self.asp_role_map[role]][1] for role in self.asp_role_map])
                    self.role_requested = [role for role in self.asp_role_map if self.current_best_asp_understanding[self.asp_role_map[role]][1] == min_conf][0]
    
            print "role_requested: " + str(self.role_requested)
            if (relatively_confident_about[self.asp_role_map["action"]] == "at"): #another set of constraints
                if (self.role_requested == "patient"): #if believe in walking, don't ask about known-to-be-False-patient
                    self.role_requested = "recipient"
#print "articulate_next_state: current_best_asp_understanding="+str(self.current_best_asp_understanding)+"; min_conf="+str(min_conf)+"; role_requested="+str(self.role_requested) #DEBUG
        ...
        print "verbal query: " + str(verbal_query) + " rel conf about: " + str(relatively_confident_about)
        return verbal_query,relatively_confident_about
    ...
    def write_asp_goal_from_apr_tuple(self, apr):
        ...
        if (apr["action"] == "served"):
            if isinstance(apr["recipient"], bool) and isinstance(apr["patient"], bool):
                asp_function = apr["action"]+"("+"False"+","+"False"+",n)"
            elif isinstance(apr["recipient"], bool):
                asp_function = apr["action"]+"("+"False"+","+apr["patient"].split(':')[0]+",n)"
            elif isinstance(apr["patient"], bool):
                asp_function = apr["action"]+"("+apr["recipient"].split(':')[0]+","+"False"+",n)"
            else:
                asp_function = apr["action"]+"("+apr["recipient"].split(':')[0]+","+apr["patient"].split(':')[0]+",n)"
        elif (apr["action"] == "at"):
            if isinstance(apr["recipient"], bool):
                asp_function = apr["action"]+"("+"False"+",n)"
            else:
    ...
    def get_command_from_user(self, user_response=None):
        ...
        while (confident_in_understanding == False):
            ...
            #system's turn
            vocalize_for_user,self.roles_relatively_confident_about = self.articulate_next_state_entr()
